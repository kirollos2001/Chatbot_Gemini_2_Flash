...
def cache_units_from_db():
    try:
        units = enrich_units_with_names()
        save_to_cache("units.json", units)
        logging.info(f"✅ Saved {len(units)} units to cache")
    except Exception as e:
        logging.error(f"❌ Error caching units: {e}")
...
def cache_new_launches_from_db():
    ...
    try:
        ...
        logging.info(f"✅ Saved {len(new_launches)} new launches to cache")
    except Exception as e:
...
def sync_conversations_to_db():
    ...
    for convo in convos:
        try:
            insert_query = """
                INSERT INTO conversations (conversation_id, user_id, description)
                VALUES (%s, %s, %s)
            """
            values = (
